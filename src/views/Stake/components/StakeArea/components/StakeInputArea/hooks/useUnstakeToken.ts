...
export const useUnstakeToken = (fromToken: "sGDAO" | "xGDAO") => {
  ...
  // const addresses = fromToken === "sGDAO" ? SOHM_ADDRESSES : GOHM_ADDRESSES;
  ...
  return useMutation<ContractReceipt, EthersError, string>(
    async amount => {
      ...
      const _amount = new DecimalBigNumber(amount, fromToken === "xGDAO" ? 18 : 9);
      ...
      if (!goerliContract) throw new Error(`Please switch to the Ethereum network to unstake your` + ` ${fromToken}`);

      if (!address) throw new Error(`Please refresh your page and try again`);

      const shouldRebase = fromToken === "sGDAO";

      const trigger = false; // was true before the mint & sync distributor change

      const transaction = await goerliContract.unstake(address, _amount.toBigNumber(), trigger, shouldRebase);
    ...
    },
    {
      ...
      onSuccess: async (tx, amount) => {
        trackGAEvent({
          ...
          value: new DecimalBigNumber(amount, fromToken === "xGDAO" ? 18 : 9).toApproxNumber(),
