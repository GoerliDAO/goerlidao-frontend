...
export const getBalancerLPToken = async ({ address, networkId }: { address: string; networkId: NetworkId }) => {
  ...
  try {
    ...
    const vault = BALANCER_VAULT.getEthersContract(NetworkId.TESTNET_GOERLI);
    ...
    const tokens = await Promise.all([
      ...addresses.map(address => getTokenByAddress({ address, networkId: NetworkId.TESTNET_GOERLI })),
    ]);
