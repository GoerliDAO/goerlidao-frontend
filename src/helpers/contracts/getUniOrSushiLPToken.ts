...
export const getUniOrSushiLPToken = async ({ address, networkId }: { address: string; networkId: NetworkId }) => {
  if (networkId !== NetworkId.TESTNET_SEPOLIA && networkId !== NetworkId.TESTNET_GOERLI)
    throw new Error("Not implemented");

  try {
    const provider = Providers.getStaticProvider(NetworkId.TESTNET_GOERLI);
    ...
    const zeroAddress = tokenZero.addresses[NetworkId.TESTNET_GOERLI];
    const oneAddress = tokenOne.addresses[NetworkId.TESTNET_GOERLI];

    const lpToken = new Token({
      decimals,
      factory: PairContract__factory,
      addresses: { [NetworkId.TESTNET_GOERLI]: address },
      name: `${tokenZero.name}-${tokenOne.name} LP`,
      icons: [...tokenZero.icons, ...tokenOne.icons],
      purchaseUrl: name.includes("Uniswap")
        ? `https://app.uniswap.org/#/add/v2/${zeroAddress}/${oneAddress}?chain=TESTNET_GOERLI`
